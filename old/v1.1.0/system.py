...
def story_SF(result_xlsx_path, input_xlsx_path, yticks=2, xlim=70000):
    ''' 
...
#%% Analysis Result 불러오기
    ...
    for i in to_load_list:
        shear_force_data_temp = pd.read_excel(i, sheet_name='Structure Section Forces'
                                              , skiprows=[0,2], usecols=[0,3,5,6,7])
        shear_force_data = pd.concat([shear_force_data, shear_force_data_temp])
...
#%% Story 정보 load
    story_info = pd.read_excel(input_xlsx_path, sheet_name='Story Data', skiprows=3, usecols=[0,1,2], keep_default_na=False)
...
#%% Analysis Result 불러오기
    ...
    for i in to_load_list:
        drift_data_temp = pd.read_excel(i, sheet_name='Drift Output'
                                             , skiprows=[0, 2], header=0
                                             , usecols=[0, 1, 3, 5, 6]) # usecols로 원하는 열만 불러오기
        drift_data = pd.concat([drift_data, drift_data_temp])        
    
    drift_data = drift_data.sort_values(by=['Load Case', 'Drift ID', 'Step Type']) # 지진파 순서가 섞여있을 때 sort
...
#%% Drift Name에서 story, direction 뽑아내기
    drift_name = drift_data['Drift Name']
...
#%% 지진파 이름 자동 생성
    ...
    load_name_list = drift_data['Load Case'].drop_duplicates()
    seismic_load_name_list = [x for x in load_name_list if ('DE' in x) or ('MCE' in x)]
