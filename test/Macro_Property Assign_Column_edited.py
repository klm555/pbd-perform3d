...
########################################### 처음에 입력해야 할 부분 ############################################
### 초기 경로 설정
data_path = r'D:\이형우\성능기반 내진설계\21-GR-222 광명 4R구역 주택재개발사업 성능기반내진설계\101' # Input Sheets 및 Output Sheets 가 있는 폴더 경로
Input_sheet_name = '101D_Data Conversion_Ver.1.2.xlsx' # Input Sheets 이름
...
wall_index = 97 # wall_material_data_repeat 에서 시작하고자 하는 material name의 index 입력, 처음부터일때는 0 입력
#####################################
...
elem_info = pd.read_excel(data_path + '\\' + Input_sheet_name
                             , sheet_name = 'Output_E.Column Properties', skiprows=[0,2,3]).iloc[:,0]
...
elem_name_count = elem_info.iloc[wall_index,0].split('_') # loop 돌면서 전 row와 비교하고, 부재 바뀔 때 프레임 넘기기
# wall_num_count = 
for i in range(wall_index, len(elem_info)):
