...
input_xlsx_path = r'K:\2105-이형우\성능기반 내진설계\KHSM\103\Data Conversion_Ver.1.3M_103D.xlsx'
...
result_xlsx_path = r'D:\이형우\성능기반 내진설계\22-GR-167 김해 신문1지구 도시개발사업 A1블록 공동주택 신축 성능기반내진설계\비선형해석모델\103\KHSM_103_2_Pre-Analysis.xlsx'
...
# input_xlsx_path = r'D:\이형우\성능기반 내진설계\22-GR-167 김해 신문1지구 도시개발사업 A1블록 공동주택 신축 성능기반내진설계\105\KHSM_105_Data Conversion_Ver.1.3M_내진상세_변경.xlsx'
input_xlsx_sheet = 'Output_G.Column Properties'
result_xlsx_path = r'D:\이형우\성능기반 내진설계\22-GR-167 김해 신문1지구 도시개발사업 A1블록 공동주택 신축 성능기반내진설계\비선형해석모델\Results\101\KHSM_101_2_Pre-Analysis.xlsx'
...
# 필요한 부재만 선별
element_info_data = element_info_data[element_info_data['Group Name'] == 'COLUMN']
element_info_data['Property Name'] = element_info_data['Property Name'].str[:-1] + '_1_'

node_data = pd.read_excel(to_load_list, sheet_name='Node Coordinate Data'
                                  , skiprows=[0, 2], header=0, usecols=[1,4]) # usecols로 원하는 열만 불러오기

#%% 101동 element 이름 재명명(101동 부재 섞어서 쓰심)     ########## 허무원 ##########
node_data_101 = pd.read_excel(to_load_list, sheet_name='Node Coordinate Data'
                                  , skiprows=[0, 2], header=0, usecols=[1,2,3,4]) # usecols로 원하는 열만 불러오기
element_data_101 = pd.merge(element_info_data, node_data_101, how='left', left_on='I-Node ID', right_on='Node ID')
...
element_info_data['Property Name'] = list_101

element_info_data = pd.merge(element_info_data, node_data, how='left', left_on='I-Node ID', right_on='Node ID')

# 전단력, 부재 이름 Matching (by Element Name)
SF_ongoing = pd.merge(element_info_data.iloc[:, [1,2,4,5]], SF_info_data.iloc[:, 1:], how='left')
