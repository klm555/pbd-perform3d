...
# Analysis Result
result_path = r'L:\33. 내진성능평가 & 성능설계\02. 진행 중\21-GR-222 광명 4R구역 주택재개발사업 성능기반내진설계\결과정리\101'
result_xlsx = 'Analysis Result' # 해석결과에 공통으로 포함되는 이름 (확장자X)

# Data Conversion Sheet, Column Sheet, Beam Sheet
input_path = r'L:\33. 내진성능평가 & 성능설계\02. 진행 중\21-GR-222 광명 4R구역 주택재개발사업 성능기반내진설계\결과정리\101'
input_xlsx = '101D_Data Conversion_Shear Wall Type.xlsx'
...
if len(DE_load_name_list) != 0:
    ...
    new_name_list = []
    for idx, row in beam_rot_data_total_DE.loc[:,['Property Name', 'Story Name']].iterrows():
        name_splitted = row[0].split('_')[0]
        new_name_list.append(name_splitted + '_1_' + row[1])

    beam_rot_data_total_DE['Property Name'] = new_name_list
    #####################        
...
if len(MCE_load_name_list) != 0:
    ...
    #### OLD VERSION ####    
    # 이전 버전의 네이밍에 맞게 merge하는 방법
    beam_rot_data_total_MCE['Property Name'] = new_name_list
    #####################     
...
#### OLD VERSION ####
beam_info['Property Name'] = beam_info['Name'].str.split('_').str[0]    
beam_info['Story Name'] = beam_info['Name'].str.split('_').str[1]

story_list = []
for i in beam_info['Story Name']:    
    if '-' in i:
        story_from = i.split('-')[0]
        story_to = i.split('-')[1]
        story_name = story_info['Story Name'][::-1].tolist()

        story_range = story_name[story_name.index(story_from) : story_name.index(story_to)+1]     
        story_list.append(story_range)
    else: story_list.append([i])
    
beam_info['Story Range'] = story_list

beam_info_floor_exploded = beam_info.explode('Story Range')
beam_info_floor_exploded['Name'] = beam_info_floor_exploded['Property Name']\
                                   + '_1_' + beam_info_floor_exploded['Story Range']
beam_info_floor_exploded = beam_info_floor_exploded.iloc[:,:18]                                  
#####################    

### BR avg total에 BR criteria join(wall name 기준)
BR_avg_total = pd.merge(beam_info_floor_exploded, beam_rot_data_total_avg, how='left')
BR_avg_total.dropna(subset='MCE avg', inplace=True)
