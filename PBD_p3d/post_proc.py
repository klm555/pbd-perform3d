...
class PostProc():
    ...
    def __init__(self, input_xlsx_path, result_xlsx_path, get_base_SF=False
        ...
        if get_BR == True:
            self.beam_deform_cap = read_excel(input_xlsx_path, sheet_name='Input_C.Beam', skip_rows=[0,1,2])
            self.beam_deform_cap = self.beam_deform_cap.iloc[:,[0,90,91]]
            self.beam_deform_cap.dropna(how='all', inplace=True)
